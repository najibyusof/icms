                        ...
                        <tbody>
                            @foreach($programmes as $programme)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-secondary">{{ $programme->programme_p_l_os_count ?? 0 }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="{{ route('programmes.show', $programme) }}" class="btn btn-outline-primary" title="View">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            @can('update', $programme)
                                                <a href="{{ route('programmes.edit', $programme) }}" class="btn btn-outline-warning" title="Edit">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                            @endcan
                                            @can('delete', $programme)
                                                <form action="{{ route('programmes.destroy', $programme) }}" method="POST" style="display:inline;">
                                                    @csrf @method('DELETE')
